...
class ChatRequest(BaseModel):
    message: str
    session_id: str
    format: str = None  # Optional: 'bullet', 'paragraph', 'visualize', etc.
...
def chat(request: ChatRequest):
    try:
        ...
        # 🔍 Smart visualization via LLM extraction
        visual_keywords = ["visualize", "chart", "graph", "plot", "compare", "show as"]
        if response_format == "visualize" or any(word in message.lower() for word in visual_keywords):
            ...
            return {"error": "Could not extract chart-worthy data from your input."}

        # Handle chart formats directly (random demo charts)
